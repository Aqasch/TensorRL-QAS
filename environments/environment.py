...
class CircuitEnv():
    ...
    def __init__(self, conf, device):
        ...
        self.hamiltonian, eigvals, energy_shift, weights = __ham['hamiltonian'],__ham['eigvals'], __ham['energy_shift'], __ham['weights']
        ...
        # exit()
        true_min_eig = min(eigvals)+energy_shift
    ...
    def reset(self):
        ...
        if self.TN_init:
            ...
            for depth_no, circ in enumerate(self.depth_wise_gates):
                ...
                for c in circ:
                    ...
                    if c.op.name != 'cx':
                        indx = str(c.qargs).index('), ')
                        if len(str(c.qargs)) <=37:
                            gate_pos = int(str(c.qargs)[indx+3])
                        else:
                            gate_pos = int(str(c.qargs)[indx+3] + str(c.qargs)[indx+4])
                        
                        if self.zero_param_init:
                            gate_param = 0
                        else:
                            gate_param = c.op.params[0]
                        print(gate_param, c.op.name, gate_pos)
                        if c.op.name == 'rx':
                            state[depth_no][self.num_qubits][gate_pos] = 1
                            state[depth_no][self.num_qubits+3][gate_pos] = gate_param
                        elif c.op.name == 'ry':
                            state[depth_no][self.num_qubits+1][gate_pos] = 1
                            state[depth_no][self.num_qubits+3+1][gate_pos] = gate_param
                        elif c.op.name == 'rz':
                            state[depth_no][self.num_qubits+2][gate_pos] = 1
                            state[depth_no][self.num_qubits+3+2][gate_pos] = gate_param
                    else:
                        if self.num_qubits <= 10:

                            indx = str(c.qargs[0]).index('), ')
                            ctrl = int(str(c.qargs[0])[indx+3])
                            targ = int(str(c.qargs[1])[indx+3])
                        # 
                        else:
                            if len(str(c.qargs[0])) <= 34:
                                ...
                                ctrl = int(str(c.qargs[0])[indx+3] + str(c.qargs[0])[indx+4])

                            if len(str(c.qargs[1])) == 34:
                                indx = str(c.qargs[1]).index('), ')
                                targ = int(str(c.qargs[1])[indx+3])
                            else:
                                targ = int(str(c.qargs[1])[indx+3] + str(c.qargs[1])[indx+4])
                            print(ctrl, targ)
        ...
        print(self.make_circuit(thetas))
        self.step_counter = -1
